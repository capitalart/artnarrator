# FULL CODE STACK (SAT-16-AUGUST-2025-12-02-PM)
...
def _process_upload_file(file_storage):
    ...
    try:
        ...
        # 4) Read dimensions and aspect safely
        try:
...
# 1. Imports
# ===========================================================================
import argparse, base64, json, logging, os, re, sys, traceback, shutil
from pathlib import Path
...
def get_aspect_ratio(image_path: Path) -> str:
    ...
    # Use lightweight dimension reader to avoid opening huge images in Pillow
...
def get_dominant_colours(img_path: Path, n: int = 2) -> list[str]:
    ...
    opt_img_path = None
    try:
        # Create a small optimized working copy to avoid opening huge originals in memory
        temp_dir = config.UNANALYSED_ROOT / "temp"
        temp_dir.mkdir(parents=True, exist_ok=True)
        opt_img_path = make_working_copy(img_path, temp_dir, long_edge=300, quality=85)
        with Image.open(opt_img_path) as img:
...
def generate_ai_listing(image_path: Path, aspect: str, assigned_sku: str) -> tuple[dict, str]:
    ...
    # Create an optimized working copy to send to the AI service (low memory and faster uploads)
    temp_dir = config.UNANALYSED_ROOT / "temp"
    ...
    # Some AI clients accept file objects or image paths; this code embeds a base64 data URI
...
def add_to_mockup_queue(artwork_path: str) -> bool:
    """Append an artwork path to the pending mockups queue file.

    This is a lightweight persistent queue used by the web routes to
    hand off files to the composite generation pipeline.
    """
    try:
        ...
        logger.error(f"Failed to queue mockup for {artwork_path}: {e}")
        return False
...
def analyze_single(image_path: Path):
    ...
    # Use a working copy for colour detection to limit memory use
    temp_dir = config.UNANALYSED_ROOT / "temp"
    working_for_colour = make_working_copy(orig_path, temp_dir, long_edge=2400, quality=90)
    primary_colour, secondary_colour = get_dominant_colours(Path(working_for_colour), 2)
...
def create_seo_filename(slug: str, sku: str) -> str:
    """Return SEO filename limited to 70 chars: [slug]-by-robin-custance-rjc-[SKU].jpg"""
    safe_slug = re.sub(r"[^A-Za-z0-9\-]", "-", slug).strip("-")
    base = f"{safe_slug}-by-robin-custance-rjc-{sku}".lower()
    # Ensure total filename <= 70 chars (excluding extension)
    if len(base) > 70:
        base = base[:70]
    return f"{base}.jpg"
...
def process_sku(sku: str, json_output: bool = False) -> dict:
    """Process an artwork by SKU: run analysis, move files to processed, and emit listing."""
    logger.info(f"Processing SKU: {sku}")
    unan_dir = config.UNANALYSED_ROOT / sku
    if not unan_dir.exists():
        raise FileNotFoundError(f"Unanalysed folder not found for SKU: {sku}")

    analyse_path = unan_dir / f"{sku}-analyse.jpg"
    source_path = unan_dir / f"{sku}-source.jpg"
    qc_path = unan_dir / f"{sku}.qc.json"

    if not analyse_path.exists():
        raise FileNotFoundError(f"Analyse image not found for SKU: {sku}")
    if not source_path.exists():
        raise FileNotFoundError(f"Source image not found for SKU: {sku}")

    # Run AI analysis using the analyse image
    aspect = get_aspect_ratio(analyse_path)
    ai_listing, raw_response = generate_ai_listing(analyse_path, aspect, sku)
    ...
    # Copy source image to processed folder as final SEO filename
    final_image_path = processed_folder / seo_filename
    shutil.copy2(str(source_path), str(final_image_path))

    # Merge QC and AI data into listing JSON
    qc_data = {}
    if qc_path.exists():
        try:
    ...
    full_listing["openai_analysis"] = {
        "time_responded": _dt.datetime.now(_dt.timezone.utc).isoformat(),
        "api_response": raw_response[:500] + "..." if raw_response else ""
    }
    ...
    try:
        ...
        if qfile.exists():
            ...
            current = []
        current.append(str(processed_folder))
        qfile.write_text(json.dumps(current, indent=2), encoding="utf-8")
    except Exception as e:
        logger.error(f"Failed to add processed folder to mockup queue: {e}")

    # Cleanup unanalysed folder
    try:
        shutil.rmtree(unan_dir)
    except Exception as e:
        logger.warning(f"Failed to delete unanalysed folder for {sku}: {e}")
...
def main():
    ...
    try:
        # If the input looks like a SKU (starts with SKU prefix), process by SKU
        if str(args.input).upper().startswith(config.SKU_CONFIG["PREFIX"]):
            ...
            # Legacy mode: accept image path and reuse analyze_single
            result = analyze_single(Path(args.input))
            # Legacy result: still return a JSON-compatible structure
            result = {"success": True, "listing": result.get("listing"), "sku": result.get("sku")}
...
def _process_queued_artwork(img_path_str: str, total_in_queue: int, current_index: int):
    ...
    # Open the working copy using a context manager so it's closed promptly.
    with Image.open(working_art_path) as art_img:
        mockup_entries = []
    ...
    for i, (cat_dir, mockup_file) in enumerate(selections):
        ...
        try:
            ...
            # open mockup file (these are small PNG templates) and use immediately
            with Image.open(mockup_file) as mockup_img:
