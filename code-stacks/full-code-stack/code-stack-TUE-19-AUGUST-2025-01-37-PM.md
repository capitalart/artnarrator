# FULL CODE STACK (TUE-19-AUGUST-2025-01-37-PM)
...
def _process_upload_file(file_storage):
    ...
    NOTE: Analysis is NOT auto-triggered unless AUTO_ANALYZE_ON_UPLOAD=1 is set.
    """
    ...
    try:
        ...
        # Gate auto-analysis behind an env flag (default OFF)
        auto = os.getenv("AUTO_ANALYZE_ON_UPLOAD", "0") == "1"
        if auto:
            subprocess.Popen(
                [sys.executable, str(config.ANALYZE_SCRIPT_PATH), sku],
                cwd=str(config.BASE_DIR)
            )
            log_action(
                "upload", sku, session.get("username", "system"),
                f"Queued for analysis (AUTO_ANALYZE_ON_UPLOAD=1). Files in {dest_folder.name}"
            )
        else:
            log_action(
                "upload", sku, session.get("username", "system"),
                f"Ready to analyze (no auto-analysis). Files in {dest_folder.name}"
            )
