# FULL CODE STACK (MON-18-AUGUST-2025-01-16-PM)
...
bp = Blueprint("artwork", __name__)
logger = logging.getLogger(__name__) # <-- FIX: DEFINED LOGGER
...
def analyze_artwork_route(sku: str):
    ...
    src_folder = config.UNANALYSED_ROOT / sku
    ...
    try:
        ...
        ai_listing = analysis_result.get("ai_listing", {})
        seo_folder_name = Path(ai_listing.get("seo_filename", "")).stem
        if not seo_folder_name:
            raise RuntimeError("Analysis did not return a valid seo_filename.")

        dest_folder = config.PROCESSED_ROOT / seo_folder_name
        if dest_folder.exists():
            shutil.rmtree(dest_folder)
        
        # Move the entire folder first
        shutil.move(str(src_folder), str(dest_folder))
        log_action("move", sku, session.get("username"), f"Moved to processed/{seo_folder_name}")

        # --- FIX: RENAME ASSETS INSIDE THE MOVED FOLDER ---
        # Rename source image
        old_source_path = dest_folder / f"{sku}-source.jpg"
        new_source_path = dest_folder / f"{seo_folder_name}-original.jpg"
        if old_source_path.exists():
            old_source_path.rename(new_source_path)

        # Rename thumbnail
        old_thumb_path = dest_folder / f"{sku}-thumb.jpg"
        new_thumb_path = dest_folder / f"{seo_folder_name}-thumb.jpg"
        if old_thumb_path.exists():
            old_thumb_path.rename(new_thumb_path)
        # --- END FIX ---
        
        listing_path = dest_folder / f"{seo_folder_name}-listing.json"
        full_listing.update(ai_listing)
        full_listing["sku"] = sku
        full_listing["aspect_ratio"] = analysis_result.get("aspect_ratio")
        full_listing["openai_analysis"] = analysis_result.get("openai_analysis", {})
        listing_path.write_text(json.dumps(full_listing, indent=2), encoding="utf-8")
        
        add_to_mockup_queue(str(new_source_path))
    ...
    redirect_filename = full_listing.get("seo_filename", f"{seo_folder_name}.jpg")
...
def analyze_artwork_by_filename(aspect, filename):
    ...
    try:
        ...
        try:
            pf = analysis_result.get("processed_folder")
            if pf: add_to_mockup_queue(pf)
            _generate_composites(uuid.uuid4().hex)
        except Exception:
            logger.exception("Composite generation subprocess failed; continuing.")
    ...
    final_aspect = analysis_result.get("aspect_ratio", aspect)
    redirect_filename = f"{seo_folder}.jpg"
    redirect_url = url_for("artwork.edit_listing", aspect=final_aspect, filename=redirect_filename)
...
def edit_listing(aspect, filename):
    ...
    stage = "vault" if is_locked_in_vault else "processed"
...
def _process_upload_file(file_storage):
    ...
    content_length = getattr(file_storage, 'content_length', 0)
...
---
/* static/css/admin-styles.css */
/* Styles for the admin audit trail page */
...
/* Overlay styles: hidden by default, shown when parent has .swapping */
.mockup-overlay {
  position: absolute;
  inset: 0;
  display: flex;
...
/* Swap status text inside the control area */
.swap-status {
...
/* Spinner fallback for better contrast in dark mode */
@media (prefers-color-scheme: dark) {
  .mockup-overlay { background: rgba(0,0,0,0.65); }
  .swap-spinner { border-top-color: #fff; border-color: rgba(255,255,255,0.25); }
}
...
<div class="gallery-section">
      ...
      <div class="gallery-card" data-filename="{{ art.filename }}" data-aspect="{{ art.aspect }}" data-base="{{ art.base }}">
        ...
        <div class="card-details">
          ...
          <div class="button-row">
            ...
            <form method="post" action="{{ url_for('artwork.delete_unanalysed', base_name=art.base) }}" style="display:inline;" onsubmit="return confirm('Delete this artwork and all files? This cannot be undone.');">
  ...
  <div class="gallery-card" data-filename="{{ art.filename }}" data-aspect="{{ art.aspect }}" data-base="{{ art.seo_folder }}">
        <div class="card-thumb">
          <img class="card-img-top"
      ...
      <div class="mockup-preview-grid">
        {% for m in mockups %}
          <div class="mockup-card" id="mockup-card-{{ m.index }}">
            {# Accessibility: per-card live region for swap status updates (screen-reader only) #}
            <div class="sr-only swap-live" aria-live="polite" aria-atomic="true"></div>
            {% if m.exists and m.thumb_exists %}
              ...
              <a href="{{ full_url ~ '?t=' ~ cache_ts }}" class="mockup-img-link" id="mockup-link-{{ m.index }}" data-img="{{ full_url ~ '?t=' ~ cache_ts }}">
                ...
                <div class="mockup-overlay" aria-hidden="true">
                  <img src="{{ url_for('static', filename='icons/svg/light/arrows-clockwise-light.svg') }}" class="spinner-icon" alt="">
                </div>
              </a>
              <div class="thumb-note">Click to preview full-size</div>
            {% else %}
              {# Ensure the overlay exists even for placeholders so swapping UI is consistent #}
              <a href="#" class="mockup-img-link" id="mockup-link-{{ m.index }}" data-img="">
                <img id="mockup-img-{{ m.index }}" src="{{ url_for('static', filename='img/default-mockup.jpg') }}" class="mockup-thumb-img" alt="Default mockup placeholder">
                <div class="mockup-overlay" aria-hidden="true" style="display:none;">
                  <img src="{{ url_for('static', filename='icons/svg/light/arrows-clockwise-light.svg') }}" class="spinner-icon" alt="">
                </div>
              </a>
            ...
            {% if categories %}
              <div class="swap-controls">
                ...
                <div class="swap-btn-container">
                  <button type="button" class="btn btn-sm swap-btn" data-index="{{ m.index }}" aria-controls="mockup-card-{{ m.index }}">
                    <span class="swap-spinner" aria-hidden="true"></span>
                    <span class="swap-label">Swap</span>
                  </button>
                  <div class="swap-status" aria-hidden="true" style="display:none; margin-top:.4rem; font-size:0.9em;"></div>
                </div>
              </div>
            {% endif %}
          </div>
...
<div class="home-hero">
  <h1>Welcome to ArtNarrator Listing Machine</h1>
  <p class="home-intro">
...
<div class="workflow-row">
  <a href="{{ url_for('artwork.upload_artwork') }}" class="workflow-btn">
    <img src="{{ url_for('static', filename='icons/number-one.svg') }}" class="step-btn-icon" alt="Step 1" />
    Upload Artwork
  </a>
    ...
    <div id="overlay-menu" class="overlay-menu">
        ...
        <nav class="overlay-nav">
            <div class="nav-column">
                ...
                <ul><li><a href="{{ url_for('artwork.upload_artwork') }}">Upload Artwork</a></li><li><a href="{{ url_for('artwork.artworks') }}">All Artworks</a></li><li><a href="{{ url_for('artwork.finalised_gallery') }}">Finalised</a></li><li><a href="{{ url_for('artwork.locked_gallery') }}">Locked</a></li></ul>
            </div>
            <div class="nav-column">
                <h3>Workflow &amp; Tools</h3>
                <ul><li><a href="{{ url_for('artwork.select') }}">Composites Preview</a></li><li><a href="{{ url_for('mockup_admin.dashboard') }}">Mockup Admin</a></li><li><a href="{{ url_for('coordinate_admin.dashboard') }}">Coordinate Generator</a></li><li><a href="{{ url_for('artwork.select') }}">Mockup Selector</a></li></ul>
            </div>
            <div class="nav-column">
                <h3>Exports &amp; Admin</h3>
                <ul><li><a href="{{ url_for('exports.sellbrite_management') }}">Sellbrite Management</a></li><li><a href="{{ url_for('admin.dashboard') }}">Admin Dashboard</a></li><li><a href="{{ url_for('audit.dashboard') }}">System Audit</a></li><li><a href="{{ url_for('admin.security_page') }}">Admin Security</a></li><li><a href="{{ url_for('gdws_admin.editor') }}">Description Editor (GDWS)</a></li>{% if session.logged_in %}<li><a href="{{ url_for('auth.logout') }}">Logout</a></li>{% else %}<li><a href="{{ url_for('auth.login') }}">Login</a></li>{% endif %}</ul>
            </div>
        </nav>
    </div>
    ...
    <footer class="site-footer">
        <div class="footer-grid">
            <div class="footer-column">
                <h4>Navigate</h4>
                <ul><li><a href="{{ url_for('artwork.home') }}">Home</a></li><li><a href="{{ url_for('auth.login') }}">Login</a></li></ul>
            </div>
            <div class="footer-column">
                <h4>Artwork &amp; Gallery</h4>
                <ul><li><a href="{{ url_for('artwork.upload_artwork') }}">Upload Artwork</a></li><li><a href="{{ url_for('artwork.artworks') }}">Artworks</a></li><li><a href="{{ url_for('artwork.finalised_gallery') }}">Finalised</a></li><li><a href="{{ url_for('artwork.locked_gallery') }}">Locked</a></li></ul>
            </div>
            <div class="footer-column">
                <h4>Workflow &amp; Tools</h4>
                <ul><li><a href="{{ url_for('artwork.select') }}">Composites Preview</a></li><li><a href="{{ url_for('artwork.select') }}">Mockups</a></li></ul>
            </div>
            <div class="footer-column">
                <h4>Exports &amp; Admin</h4>
                <ul><li><a href="{{ url_for('exports.sellbrite_management') }}">Sellbrite Management</a></li><li><a href="{{ url_for('admin.dashboard') }}">Admin Dashboard</a></li><li><a href="{{ url_for('admin.security_page') }}">Admin Security</a></li><li><a href="{{ url_for('gdws_admin.editor') }}">Description Editor (GDWS)</a></li></ul>
            </div>
        </div>
...
---
## tests/test_run_ai_analysis_exceptions.py
---
import subprocess
from pathlib import Path
import pytest
