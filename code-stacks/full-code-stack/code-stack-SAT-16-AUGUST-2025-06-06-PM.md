# FULL CODE STACK (SAT-16-AUGUST-2025-06-06-PM)
...
def analyze_artwork_route(sku: str):
    ...
    try:
        ...
        if isinstance(analysis_result, list):
            if len(analysis_result) == 1 and isinstance(analysis_result[0], dict):
                analysis_result = analysis_result[0]
            else:
...
def analyze_artwork_by_filename(aspect, filename):
    ...
    try:
        ...
        # Queue mockup generation and attempt to trigger composite generation.
...
logger = setup_logger(__name__, "ANALYZE")
...
def generate_ai_listing(image_path: Path, aspect: str, assigned_sku: str) -> tuple[dict, str]:
    ...
    Real implementation calls OpenAI/Gemini; keep stub so imports succeed in tests.
    """
...
def analyze_single(image_path: Path) -> dict:
    ...
    # If the parent folder already looks like a SKU, keep it; otherwise
    # allocate a fresh SKU so downstream processing uses the SKU-first contract.
    ...
    # Use working copy for colour detection
    # Attempt to use an existing canonical original name if present; otherwise
    # treat the provided image_path as the source and copy it into the processed
    # folder below.
    orig = image_path
    try:
        # If there is a sku-original.jpg in the same folder, prefer it for colour extraction
    ...
    # Create a processed folder and write a minimal listing so legacy callers
    # (and tests) that expect a processed_folder value can continue to work.
    try:
        ...
        # Copy the source image into the processed folder under the seo filename
        try:
            shutil.copy2(str(orig), str(final_image_path))
        except Exception:
            # Best-effort copy; ignore on failure
            pass
        ...
        # Queue mockup generation (best-effort)
        try:
