# FULL CODE STACK (SUN-03-AUGUST-2025-12-32-PM)
...
# Correctly import all required config variables
from config import (
    PROCESSED_ROOT, FINALISED_ROOT, UNANALYSED_ROOT, ARTWORK_VAULT_ROOT,
    BASE_DIR, ANALYSIS_STATUS_FILE, PROCESSED_URL_PATH, FINALISED_URL_PATH,
    LOCKED_URL_PATH, UNANALYSED_IMG_URL_PREFIX, MOCKUP_THUMB_URL_PREFIX,
    COMPOSITE_IMG_URL_PREFIX, MOCKUPS_INPUT_DIR, MOCKUPS_CATEGORISED_DIR
)
import scripts.analyze_artwork as aa
from scripts import signing_service
...
from .utils import (
    ...
    sync_filename_with_sku, is_finalised_image, get_allowed_colours,
    load_json_file_safe, update_listing_paths
)
...
def analyze_artwork_route(aspect, filename):
    ...
    if not src_path:
        try:
            seo_folder = utils.find_seo_folder_from_filename(aspect, filename)
            # This path is temporary; it might not exist if the filename has changed.
            potential_path = PROCESSED_ROOT / seo_folder / f"{seo_folder}.jpg"
...
def edit_listing(aspect, filename):
    ...
    if request.method == "POST":
        # FIX #1: This now correctly reads the 'images' textarea from the form and saves it.
        form_data = {
            "title": request.form.get("title", "").strip(),
            "description": request.form.get("description", "").strip(),
            "tags": [t.strip() for t in request.form.get("tags", "").split(',') if t.strip()],
...
def finalise_artwork(aspect, filename):
    try:
        seo_folder, _, listing_file_path, finalised = resolve_listing_paths(aspect, filename)
        if finalised:
    ...
    try:
        # FIX #2: Add a safety check. Before moving anything, ensure the image list
        # has been populated and saved. This prevents the crash.
        data = utils.load_json_file_safe(listing_file_path)
        if "images" not in data or not data["images"]:
            flash("Cannot finalise: Image URLs are missing. Please click 'Update Image URLs' and then 'Save Changes' before finalising.", "danger")
            return redirect(url_for("artwork.edit_listing", aspect=aspect, filename=filename))

        if final_dir.exists(): shutil.rmtree(final_dir)
        shutil.move(str(processed_dir), str(final_dir))

        listing_file_in_final = final_dir / listing_file_path.name
...
# ===========================================================================
# 10. Listing File Path Management (NEWLY ADDED)
# ===========================================================================
