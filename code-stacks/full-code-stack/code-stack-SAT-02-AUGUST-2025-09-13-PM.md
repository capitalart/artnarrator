# FULL CODE STACK (SAT-02-AUGUST-2025-09-13-PM)
...
def analyze_artwork_route(aspect, filename):
    ...
    # --- START: MODIFIED LOGIC FOR RE-ANALYSIS ---
    ...
    # --- END: MODIFIED LOGIC FOR RE-ANALYSIS ---
    ...
    try:
        ...
        # Cleanup for fresh analysis from unanalysed folder
        if config.UNANALYSED_ROOT in src_path.parents:
            shutil.rmtree(src_path.parent, ignore_errors=True)
            log_action("cleanup", src_path.parent.name, session.get("username"), "Deleted unanalysed artwork folder.")
            logger.info(f"Cleaned up unanalysed source folder: {src_path.parent}")
        
        # --- START: NEW CLEANUP LOGIC FOR RE-ANALYSIS ---
        # If this was a re-analysis, delete the old folder now that the new one is created.
        ...
        # --- END: NEW CLEANUP LOGIC FOR RE-ANALYSIS ---
...
def edit_listing(aspect, filename):
    ...
    if request.method == "POST":
        # FIX #1: Added the 'images' field to the form data being collected
        # so that it is correctly saved to the JSON file.
        form_data = {
            "title": request.form.get("title", "").strip(),
            "description": request.form.get("description", "").strip(),
            "tags": [t.strip() for t in request.form.get("tags", "").split(',') if t.strip()],
...
# 11. Artwork Finalisation and Gallery Routes
# ===========================================================================
# FIX #2: Changed the route to only accept POST requests.
# This prevents errors if the user accidentally tries to access the URL directly.
